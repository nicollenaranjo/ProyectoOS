...
// thread admin
static void *threadAdmin(void *arg) {
    ...
    while(1) 
    {
        ...
        while (!found)
        {
            ...
            while((readbytes = (ordinaryPipe2[0], buffer2, SIZE)) > 0)
            {
...
// thread suscribe
static void *threadSuscribe(void *arg){
    ...
    while(1) 
    {
        ...
        if (recv(clientSock, clientMessage, 2000, 0) > 0) 
        {
            ...
            while (!found)
            {
                while((readbytes = (ordinaryPipe1[0], buffer1, SIZE)) > 0)
                {
...
int main(int argc , char *argv[]) 
{
    ...
    if (t != 0)
        handle_error_en(s, "threadSuscribe");
